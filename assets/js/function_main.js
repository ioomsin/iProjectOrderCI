...
$(function(){
   ...
   $('body').on('click','.links', function(e) {
	   ...
	   if(url !='#' && url !=undefined){
		   //var title=$(this).html();
		   //$("title").html(title);
		   window.history.pushState(url, null, url); //����¹ url 
...
function loadview(url){
		$.ajax({
			  ...
			  beforeSend: function(data){	
					createCookie('notheme','1', 1); //��������ʴ� Theme (������Ŵ Ajax ���ʴ�� #show_detail)
					block(); // #show_detail
			  },
			  success: function(data){
					deleteCookie('notheme'); //¡��ԡ����������ʴ� Theme
				 	unblock();
...
window.alert = function(message,alert_type){
	...
	if($('.sweet-alert').hasClass('showSweetAlert')==true){ //��� SweetAlert �Դ���� ��� Delay 
		setTimeout(function(){
			swal({
			  title: '����͹',
			  text: message,
...
function notifys(message, type, align, valign, second){
	...
	if(second==undefined || second==''){ second=3000; } //3 �Թҷ�
	...
	$.notify({
		...
		type: type,
		delay: second,
		allow_dismiss: true, //������ Close
		mouse_over: 'pause', //���������� Hover �������͹ 	
		placement: {
			from: valign,
			align: align
		},
